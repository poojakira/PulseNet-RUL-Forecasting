...
- `torch` 2.12.0 → 2.13.0 and `setuptools` 81.0.0 → 83.0.0

### Accepted residual advisories (no compatible fix available)

The CI audit accepts only the following residual advisory:

| ID | Package | Reason accepted |
|----|---------|-----------------|
| PYSEC-2026-1325 | ecdsa 0.19.2 | No patched release is available; transitive dependency of `python-jose[cryptography]`. |

This will be re-evaluated whenever a compatible upstream fix becomes available.
