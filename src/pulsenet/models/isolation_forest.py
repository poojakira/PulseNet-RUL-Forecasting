    ...
    def save(self, path: Union[Path, str]) -> None:
        ...
        if path.suffix == ".skops":
            try:
                ...
                skio.dump(payload, path)
            except ImportError as exc:
                raise ImportError(
                    "skops is required to save .skops files. Install with: pip install skops"
                ) from exc
        else:
    ...
    def load(self, path: Union[Path, str], trusted: bool = False) -> None:
        ...
        Args:
            ...
            trusted: If False, raises ValueError for joblib files.
                     ...
                     For skops files, reviewed untrusted types require trusted=True.
        """
        path = Path(path)
        ...
        if path.suffix == ".skops":
            try:
                ...
                untrusted_types = skio.get_untrusted_types(file=path)
                if untrusted_types and not trusted:
                    raise ValueError(
                        "Loading this skops model requires trusted=True after "
                        "reviewing the untrusted types reported by skops."
                    )
                data = skio.load(path, trusted=untrusted_types)
            except ImportError as exc:
                raise ImportError(
