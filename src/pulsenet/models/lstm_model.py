...
# Disable oneDNN for LSTM on Windows (prevents "could not create primitive descriptor" error)
...
if TORCH_AVAILABLE:
    # Disable oneDNN for LSTM (prevents "could not create primitive descriptor" on Windows)
    ...
    # Provide stub classes for type checking when torch is not available
    class _LSTMAutoencoder:  # type: ignore[no-redef]
        pass

    class LSTMModel(BaseAnomalyModel):  # type: ignore[no-redef]
        name = "lstm"
