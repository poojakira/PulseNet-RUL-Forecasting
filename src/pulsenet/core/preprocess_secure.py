...
if not os.path.exists("train_FD001.txt") or not os.path.exists("test_FD001.txt"):
    ...
    print(f"Encrypting {train_df.shape[0]} training rows and {test_df.shape[0]} testing rows...")
    ...
    def encrypt_df(df, output_name):
        ...
        df_enc = df.apply(lambda x: x.astype(str).apply(lambda val: cipher.encrypt(val.encode()).decode()))
