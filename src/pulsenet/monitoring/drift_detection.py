...
DRIFT_DETECTED = Counter(
    "pulsenet_drift_detected_total", "Total drift detections", ["feature", "drift_type"]
)
DRIFT_SCORE = Gauge(
    "pulsenet_drift_score", "Current drift score", ["feature", "metric"]
)
DRIFT_CHECK_DURATION = Histogram(
    "pulsenet_drift_check_duration_seconds", "Time spent on drift checks"
)
MODEL_PERFORMANCE = Gauge(
    "pulsenet_model_performance", "Model performance metrics", ["metric"]
)
RETRAINING_TRIGGERED = Counter(
    "pulsenet_retraining_triggered_total", "Retraining pipeline triggers"
)
...
class EvidentlyDriftDetector:
    ...
    def __init__(
        ...
        if not EVIDENTLY_AVAILABLE:
            raise RuntimeError(
                "Evidently AI not installed. Install with: pip install evidently"
            )
        ...
        self.drift_report = Report(
            metrics=[
                ...
                *[
                    ...
                    for col in self.reference_data.columns[:20]
                ],
            ]
        )
    ...
    def check_drift(
        ...
        try:
            ...
            results = {
                ...
                "n_drifted_features": report_dict["metrics"][0]["result"][
                    "number_of_drifted_columns"
                ],
                "total_features": report_dict["metrics"][0]["result"][
                    "number_of_columns"
                ],
                "drift_share": report_dict["metrics"][0]["result"][
            ...
            for metric in report_dict["metrics"][1:]:
                if metric["metric"] == "ColumnDriftMetric":
                    ...
                    DRIFT_SCORE.labels(feature=col, metric="drift_score").set(
                        drift_score
                    )
            ...
            results["tests"] = {
                "passed": all(
                    t["status"] == "SUCCESS" for t in test_dict.get("tests", [])
                ),
    ...
    def _generate_alert(
        ...
        log.warning(
            f"DRIFT ALERT: {feature} (score={drift_score:.4f}, severity={severity.value})"
        )
    ...
    def run_continuous(
        self, data_provider: Callable[[], pd.DataFrame], interval_seconds: int = 3600
    ) -> None:
        ...
        while True:
            try:
                ...
                if (
                    ...
                    log.warning(
                        f"Insufficient data for drift check: {len(current_data) if current_data is not None else 0} samples"
                    )
            except Exception as e:
        ...
        for col in current_data.select_dtypes(include=[np.number]).columns:
            ...
            results[col] = {
                ...
                "drift_detected": (
                    ...
                    or ks_stat > self.config.ks_threshold
...
class ModelPerformanceMonitor:
    ...
    def __init__(
        self, model, reference_predictions: np.ndarray, reference_labels: np.ndarray
    ):
        self.model = model
        ...
        if len(np.unique(y)) <= 10:
            ...
            try:
                metrics["roc_auc"] = float(
                    roc_auc_score(y, self.model.predict_proba(X), multi_class="ovr")
                )
            except Exception:
                pass
        else:
        ...
        for metric_name, value in metrics.items():
            if metric_name in ref_metrics:
                ...
                if metric_name in ("accuracy", "f1_macro", "roc_auc"):
                    degradation = (
                        (ref_value - value) / ref_value if ref_value > 0 else 0
                    )
                    if degradation > 0.05:
                        log.warning(
                            f"Model degradation detected: {metric_name} dropped {degradation:.1%} (ref={ref_value:.4f}, current={value:.4f})"
                        )
                        MODEL_PERFORMANCE.labels(
                            metric=f"{metric_name}_degradation"
                        ).set(degradation)
                elif metric_name in ("mse", "mae"):
                    increase = (value - ref_value) / ref_value if ref_value > 0 else 0
                    if increase > 0.1:
                        log.warning(
                            f"Model degradation: {metric_name} increased {increase:.1%}"
                        )
                        MODEL_PERFORMANCE.labels(metric=f"{metric_name}_increase").set(
                            increase
                        )
